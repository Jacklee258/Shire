...
description: |
  本试卷包含基础能力与性向维度两部分内容。
  全卷共 62 题，其中单选题 60 题、简答题 2 题，预计 57 分钟完成。
...
[rubric]
`mkdir test` 用于创建目录；`touch` 用于创建普通文件，`ls` 仅用于查看目录内容，`mkdir -R` 不是此处的正确命令。
[/rubric]

## Q6 [single] (1) {answer_time=30s}
...
[rubric]
当 $x<0$ 时，$f(x)=x|x|=-x^2$；当 $x>0$ 时，$f(x)=x\ln x$。在 $x=0$ 两侧函数值都不大于 0 且 $f(0)=0$，所以 0 是极大值点；右侧差商为 $\ln x$，趋于 $-\infty$，故在 0 处不可导。
[/rubric]

## Q11 [single] (1) {answer_time=30s}
...
[rubric]
从 $V0$ 出发，按不同邻接访问顺序可得到 5 种不同的 DFS 序列：$V0,V1,V3,V2$；$V0,V2,V1,V3$；$V0,V2,V3,V1$；$V0,V3,V1,V2$；$V0,V3,V2,V1$。
[/rubric]
...
[rubric]
见解析与示意图：![第15题参考答案](./assets/a15.png)。
[/rubric]

## Q16 [single] (1) {answer_time=30s}
...
[rubric]
从主机名或域名映射到对应 IP 地址的过程称为域名解析（DNS 解析）。
[/rubric]
...
[rubric]
Docker 从镜像仓库拉取镜像的标准命令是 `docker pull`。
[/rubric]
...
## Q23 [single] (1) {answer_time=2m}

优化问题：

$$
\begin{aligned}
\max \quad & 4x_{1}+2x_{2}+4x_{3}+2x_{4}+x_{5}+x_{6} \\
\text{s.t.} \quad & 4x_{1}+4x_{2}+6x_{3}+2x_{4}+3x_{5}+2x_{6}\le 11 \\
...
[rubric]
句意是“这部小说的______使其成为畅销书”，只有 `originality` 表示“原创性、独创性”，语义最贴切。
[/rubric]
...
[rubric]
HMM 常用于语音等时序任务，可通过隐状态转移描述分段统计特性，因此 A、B、C 都可成立，答案选 D。
[/rubric]
...
[rubric]
HTTP 500 表示 `Internal Server Error`，即服务器内部处理请求时发生错误。
[/rubric]

## Q36 [single] (1) {answer_time=30s}
...
[rubric]
软件测试的核心在于设计和选择高质量测试用例，以尽可能高效地发现缺陷。
[/rubric]

## Q37 [single] (1) {answer_time=30s}
...
## Q38 [single] (1) {answer_time=30s}

云计算产品中，下列哪类角色通常是最直接的使用者？
...
[rubric]
云资源的日常部署、配置、监控与运维通常由运维人员直接完成，因此其是最直接的使用者。
[/rubric]
...
[rubric]
题目要求“每个学生所选课程个数”，应按 `student_id` 分组，再对课程数做 `count` 统计，所以选 B。
[/rubric]

## Q41 [single] (1) {answer_time=30s}
...
[rubric]
降低存款准备金率会释放银行可贷资金，通常带来信贷扩张和货币供应增加，进而推升通胀压力。
[/rubric]

## Q42 [single] (1) {answer_time=45s}

关于 GBDT+LR 的“树模型+线性模型”架构，正确的是（）

（1）GBDT 的连续输出可以不做特征转换，直接替代 LR 完成最终线性分类
（2）GBDT 常用于自动学习非线性特征组合并完成特征变换
（3）将样本落入的叶子结点做 one-hot 编码后输入 LR，是常见的 GBDT+LR 用法
（4）加入 LR 后，原始类别特征通常就不再需要任何编码处理
...
[rubric]
（2）（3）正确：GBDT 负责学习非线性特征并输出叶子索引，LR 负责基于这些离散化特征做线性分类。（1）混淆了 GBDT 输出与常见叶子 one-hot 用法；（4）表述过于绝对。
[/rubric]

## Q43 [single] (1) {answer_time=30s}
...
[rubric]
所谓 51% 攻击，指攻击者掌握全网超过一半的有效算力，从而可能重组区块、实施双花等行为。
[/rubric]

## Q44 [single] (1) {answer_time=2m}

相亲网站特征（年龄、相貌、工资、是否会编程），采用信息增益作为准则，信息增益最大的特征是（）
![第44题题干](./assets/q44.png)
...
## Q45 [single] (1) {answer_time=45s}

如下图训练过程中 test loss 越来越大，要想使其下降，下列方法中不可行的是（ ）
![第45题题干](./assets/q45.png)
...
## Q48 [single] (1) {answer_time=30s}

关于决策树算法的基本结构，以下描述正确的是（）

* A*) 在决策树中，内部结点表示特征，分支表示特征取值，叶结点表示类别
* B) 决策树中的每个内部结点都直接对应一个最终类别
* C) 决策树的叶结点只保存特征取值，不保存预测结果
* D) 决策树的所有分支都必须由连续数值特征构成

[rubric]
决策树的基本结构是“内部结点表示划分特征，分支表示取值或划分结果，叶结点表示最终类别或预测值”，因此 A 正确。
[/rubric]
...
[rubric]
按约定利息借出的资金对应债权关系，分享利润而非固定利息更接近股权关系，因此王同学是债权人，李学长是股东。
[/rubric]

## Q52 [single] (1) {answer_time=30s}
...
[rubric]
《英雄联盟》属于 MOBA 游戏，不是传统即时战略游戏（RTS）；其余三项均为典型 RTS。
[/rubric]

## Q53 [single] (1) {answer_time=30s}
...
[rubric]
参与式设计强调让真实用户直接参与设计过程，更有利于洞察隐性需求和真实使用场景。
[/rubric]

## Q54 [single] (1) {answer_time=30s}
...
[rubric]
产品语境中的 MVP 指 `Minimum Viable Product`，即“最小可行产品”。
[/rubric]

## Q55 [single] (1) {answer_time=45s}
...
[rubric]
题干把“成功差异”直接归因于遗传，D 指出了更优成长环境这一关键混杂因素，因此反驳最有力。
[/rubric]

## Q56 [single] (1) {answer_time=30s}
...
[rubric]
系统需要在四个已知类别中做监督识别，属于典型的多分类问题。
[/rubric]

## Q57 [single] (1) {answer_time=30s}
...
[rubric]
下调存贷款基准利率会降低融资成本、刺激信贷和投资，属于扩张性货币政策。
[/rubric]

## Q58 [single] (1) {answer_time=2m}
...
[rubric]
箱型图可以同时展示中位数、四分位区间、离散程度和异常值，是描述单变量分布的常用图形。
[/rubric]

## Q61 [short] {max=20, answer_time=5m}
...
## Q62 [short] {max=20, answer_time=5m}
